...
class LockdownPolicy:
    ...
    # Economic focus
    agriculture: float = attr.ib(default=1.0)
    chemical: float = attr.ib(default=1.0)
    commerce: float = attr.ib(default=1.0)
    construction: float = attr.ib(default=1.0)
    education: float = attr.ib(default=1.0)
    fin_prof_services: float = attr.ib(default=1.0)
    food_consumables: float = attr.ib(default=1.0)
    healthcare: float = attr.ib(default=1.0)
    hospitality_tourism: float = attr.ib(default=1.0)
    manufacturing: float = attr.ib(default=1.0)
    mining: float = attr.ib(default=1.0)
    engineering: float = attr.ib(default=1.0)
    media: float = attr.ib(default=1.0)
    energy: float = attr.ib(default=1.0)
    telecom: float = attr.ib(default=1.0)
    public_admin: float = attr.ib(default=1.0)
    supply_chain_ship: float = attr.ib(default=1.0)
    forest_husb_fish: float = attr.ib(default=1.0)
    textiles: float = attr.ib(default=1.0)
    transportation: float = attr.ib(default=1.0)
    utilities: float = attr.ib(default=1.0)

    # Social focus
    gathering_size: float = attr.ib(default=1.0)
    open_border: float = attr.ib(default=1.0)
    air_travel: float = attr.ib(default=1.0)
    roal_rail_travel: float = attr.ib(default=1.0)
    public_transport: float = attr.ib(default=1.0)
    curfew: float = attr.ib(default=1.0)

    ecommerce: float = attr.ib(default=1.0)
    events_allowed: float = attr.ib(default=1.0)
    worship_allowed: float = attr.ib(default=1.0)
